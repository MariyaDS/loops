...
# The for loop is concise and automatically handles the loop variable for a known number of iterations, while the while loop provides flexibility for condition-based iteration but requires manual control of the loop variable and condition.
...
# The goal is to display every employee ID number created in the loop. The variable i represents each ID number. Placing print(i) before the conditional ensures every ID is printed. If print(i) were inside the conditional, only the ID 5100 would be printed.

# ## Conclusion
# **What are your key takeaways from this lab?**
# 
# Iterative statements are crucial for automating repetitive security-related processes. For loops let you repeat a process a set number of times. While loops let you repeat a process until a specific condition is met, often using comparison operators. The < operator checks if one value is less than another, <= checks if one value is less than or equal to another, and == checks if two values are equal.
#     
# 
# 
# 
